...
vector<string> map_process(vector<string> map, int y, int x)
{
	...
	if (num_of_mines == '0')	
	    for (int i = 0; i < Maximum_Cells_Around; i++)
	    {
    		new_row = y + row_delta[i];
	    	new_column = x + col_delta[i];
		    if ((new_row != out_of_reach) && (new_column != num_of_columns) && (new_row != num_of_rows) && 
            (new_column != out_of_reach) && map[new_row][new_column] == '.')
...
int main()
{
	...
	updated_map = map_fill(map_process(map, start_y ,start_x));
